...
set.seed(123)
data <- data.frame(
  Month = rep(c("JAN", "FE", "MA", "AP"), each = 100),
  Value = c(rnorm(100, mean = 50, sd = 15),
            rnorm(100, mean = 55, sd = 20),
            rnorm(100, mean = 45, sd = 18),
            rnorm(100, mean = 60, sd = 22)),
  Category = rep(c("A", "B", "C"), times = c(130, 130, 140))
)

# Plot Ridgeline Chart (Joyplot)
ggplot(data, aes(x = Value, y = Month, fill = Category)) +
  geom_density_ridges(scale = 2, alpha = 0.8) +  # Joyplot dengan smoothing
  scale_fill_manual(values = c("#1C1F38", "#E63946", "#F76C5E")) +  # Warna sesuai contoh
  theme_minimal() +
  labs(title = "Ridgeline Plot (Joyplot)", x = "", y = "")
```
