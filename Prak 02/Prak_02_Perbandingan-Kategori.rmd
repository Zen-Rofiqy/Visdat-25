...
```{r fig.height=3.5, fig.width=10, dpi=300}
# Buat dataset tanggal & aktivitas acak
set.seed(123)
data_heatmap <- data.frame(
  Date = seq(as.Date("2024-01-01"), as.Date("2024-03-31"), by = "day"),
  Activity = sample(1:10, 91, replace = TRUE)  # Aktivitas acak dari 1-10
)

# Tambahkan kolom tambahan untuk mempermudah plotting
data_heatmap <- data_heatmap %>%
  mutate(
    Month = factor(month(Date, label = TRUE, abbr = FALSE), levels = c("January", "February", "March")),
    Weekday = factor(weekdays(Date, abbreviate = TRUE), levels = c("Su", "Mo", "Tu", "We", "Th", "Fr", "Sa")),
    Week = as.integer(format(Date, "%U")) - min(as.integer(format(Date, "%U"))) + 1
  )

# Plot Heatmap
ggplot(data_heatmap, aes(x = Weekday, y = Week, fill = Activity)) +
  geom_tile(color = "white") +  # Gunakan tile untuk membuat heatmap
  scale_fill_gradient(low = "#DCEFF3", high = "#045A8D") +  # Warna gradasi dari terang ke gelap
  facet_wrap(~ Month, ncol = 3) +  # Pisahkan berdasarkan bulan
  labs(title = "Aktivitas Harian dalam Heatmap",
       subtitle = "Visualisasi aktivitas harian dalam format kalender",
       x = "", y = "Minggu",
       fill = "Level Aktivitas") +
  theme_minimal() +
