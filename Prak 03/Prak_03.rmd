...
label_cars <- bind_rows(
  data_cars %>% slice_max(mpg, n = 1, with_ties = FALSE),  
  data_cars %>% mutate(dist_to_median = abs(mpg - median(mpg))) %>% 
    slice_min(dist_to_median, n = 1, with_ties = FALSE) %>% select(-dist_to_median),  
  data_cars %>% slice_min(mpg, n = 1, with_ties = FALSE)  
) %>%
  arrange(desc(mpg))  
...
# **Plot Parallel Coordinates dengan Label**
ggparcoord(data_cars_norm, 
  ...
  # **Pastikan warna tetap dan perbesar garis utama**
  ...
  # **Hapus legend sepenuhnya**
  guides(color = "none") +

  # **Styling tema**
  theme_minimal(base_size = 26) +  # **Perbesar font global**
  theme(
    axis.title.y = element_text(size = 24, face = "bold"),
    axis.text.y = element_text(hjust = 1, face = "bold", margin = margin(r = 5), size = 22),
    ...
    # **Geser grafik lebih ke kiri**
    plot.margin = margin(l = -10, t = 10, r = 100, b = 10)
  )
```
