...
# Ubah data ke format long untuk memudahkan penempatan label
data_long <- data_cars %>%
  pivot_longer(cols = all_of(selected_columns), names_to = "Variable", values_to = "Value") %>%
  mutate(car_name = rep(rownames(data_cars), length(selected_columns)))

# Filter hanya mobil yang di-highlight untuk diberi label
label_positions <- data_long %>%
  filter(car_name %in% label_cars$car_name) %>%
  group_by(car_name) %>%
  slice_max(Variable, n = 1)  # Ambil titik terakhir di sumbu X
```
...
ggparcoord(data_cars, 
  ...
  # Tambahkan label di ujung garis (hanya untuk 3 mobil terpilih)
  geom_text(data = label_positions, 
            aes(x = Variable, y = Value, label = car_name, color = highlight), 
