...
# Data
data_cars <- mtcars %>%
  mutate(cc = disp * 16.3871) %>% # konversi volume mesin ke cc
  select(-disp) 
...
chart <-
ggplot(data_cars, aes(x = cc, y = mpg, color = factor(cyl), shape = factor(am))) +
  # Scatterplot
  geom_point(size = 3) +  
  
  # Setting 
  labs(title = "Efek Silinder & Transmisi terhadap Efisiensi BBM",
       subtitle = "(Semakin Rendah mpg, Semakin Boros)",
