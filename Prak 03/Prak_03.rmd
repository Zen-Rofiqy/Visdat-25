...
```{r}
boros <- data_cars[which.min(data_cars$mpg), ]   
irit <- data_cars[which.max(data_cars$mpg), ]    
menengah <- data_cars[which.min(abs(data_cars$mpg - median(data_cars$mpg) )), ]   

label_cars <- rbind(boros, irit, menengah)
label_cars$car_name <- rownames(label_cars)
```
...
ggplot(data_cars,
  ...
  theme(
    ...
    legend.margin = margin(t = -10, b = -10, r = -15, l = -15)
  ) 
