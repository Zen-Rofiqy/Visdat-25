...
# **Ambil nilai cc yang sudah dinormalisasi untuk label**
label_positions <- data_cars_norm %>%
  filter(car_name %in% label_cars$car_name) %>%
  select(car_name, cc)  # Gunakan nilai cc yang sudah dinormalisasi
...
ggparcoord(data_cars, 
           ...
           scale = "uniminmax",  # Pastikan skala normalisasi
           showPoints = TRUE,    
           alphaLines = 0.6) +   
  scale_color_manual(values = color_mapping) +  
  scale_alpha_identity() +  
  labs(title = "Parallel Coordinates Plot: data_cars",
       x = "", y = "Skala Normalisasi (0 - 1)") +
