...
install_load("knitr","ggplot2","viridis","dplyr","DT","showtext","ggrepel","GGally","tibble")
...
# Tentukan posisi label di ujung garis (sumbu X terakhir)
x_max <- length(selected_columns)  # Posisi sumbu X paling kanan
label_positions <- data_cars %>%
  filter(rownames(.) %in% label_cars$car_name) %>%
  select(all_of(selected_columns)) %>%
  mutate(car_name = rownames(.),
         x = x_max,  # Posisi label di ujung kanan
         # Ambil nilai terakhir
         y = select(., all_of(selected_columns[length(selected_columns)]))[[1]])  
```
...
data_cars <- data_cars %>%
  mutate(
    ...
    highlight = factor(highlight, levels = c(label_cars$car_name, "Lainnya")),
    alpha_val = ifelse(highlight == "Lainnya", 0.2, 1)  # Buat mobil lain lebih transparan
  )

# Warna khusus: 3 mobil utama + lainnya dalam warna abu-abu
highlight_colors <- c("#21637b", "#967349", "#ad0c1b", "grey80")  
color_mapping <- setNames(highlight_colors, levels(data_cars$highlight))

# Pilih kolom berdasarkan nama
selected_columns <- c("mpg", "hp", "drat", "wt", "qsec", "cc")

# **Pastikan `label_positions` mengambil nilai dari `cc` sebagai y**
x_max <- length(selected_columns) + 0.5  # Tempatkan label sedikit di luar grafik
label_positions <- data_cars %>%
  filter(car_name %in% label_cars$car_name) %>%
  select(car_name, cc)  # Ambil nilai cc sebagai y_position

# **Plot**
ggparcoord(data_cars, 
           columns = match(selected_columns, colnames(data_cars)),   
           groupColumn = "highlight",          
           scale = "globalminmax",  # Normalisasi agar lebih proporsional
           showPoints = TRUE,    
           alphaLines = 0.6) +   # Pastikan garis terlihat
  scale_color_manual(values = color_mapping) +  
  scale_alpha_identity() +  # Pastikan alpha bekerja
  labs(title = "Parallel Coordinates Plot: data_cars",
       x = "", y = "") +

  # **Tambahkan label di ujung garis menggunakan annotate()**
  annotate("text", x = x_max, y = label_positions$cc, 
           label = label_positions$car_name, color = "black", size = 6, fontface = "bold", hjust = -0.2) +

  # **Pastikan skala y sesuai**
  ...
  theme(
    ...
    axis.text.y = element_text(hjust = 1, face = "bold", margin = margin(r = 1)),
...
ggparcoord(data_cars, 
           ...
           groupColumn = "highlight",          
           scale = "uniminmax") +  # Normalisasi 0-100
  scale_color_manual(values = color_mapping) +  
  labs(title = "Parallel Coordinates Plot: data_cars",
       x = "", y = "") +

  # Gunakan guides untuk mengatur ketebalan garis
  guides(color = guide_legend(override.aes = list(linewidth = c(2.5, 2.5, 2.5, 0.5)))) +  
